...
<body>
	...
	<ol>
		<c:forEach var="cityToHeroMap" items="${cityToHeroMap}">
			<li><b><a href="http://the-tale.org/game/map/places/${cityToHeroMap.key.id}">${cityToHeroMap.key.name}</a></b></li>
			<c:forEach var="heroGuild" items="${cityToHeroMap.value}">
				<ul>- <a href="http://the-tale.org/accounts/${heroGuild.hero.id}">${heroGuild.hero.name}</a>
					<c:choose>
